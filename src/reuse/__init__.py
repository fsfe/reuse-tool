...
_LICENSE_PATTERN = re.compile(
    r"SPDX-Licen[cs]e-Identifier: (.*?)" + _END_PATTERN, re.MULTILINE
)
_COPYRIGHT_PATTERNS = [
    re.compile(r"((?<!Copyright\s)© .*?)" + _END_PATTERN, re.MULTILINE),
    re.compile(r"(Copyright .*?)" + _END_PATTERN, re.MULTILINE),
]
_VALID_LICENSE_PATTERN = re.compile(
    r"Valid-Licen[cs]e-Identifier: (.*?)" + _END_PATTERN, re.MULTILINE
)
...
ReuseInfo = NamedTuple(
    "ReuseInfo",
    [("spdx_expressions", Set[str]), ("copyright_lines", Set[str])],
)
...
def _checksum(file_object: BinaryIO, hash_function) -> str:
    ...
    for chunk in iter(lambda: file_object.read(128 * result.block_size), b""):
        result.update(chunk)
    return result.hexdigest()


def _copyright_from_debian(path: PathLike, copyright: Copyright) -> ReuseInfo:
...
def extract_reuse_info(text: str) -> ReuseInfo:
    ...
    return ReuseInfo(license_matches, copyright_matches)
...
class Project:  # pylint: disable=unused-variable
    ...
    def reuse_info_of(
        self, path: PathLike, ignore_debian: bool = False
    ) -> ReuseInfo:
            ...
            for expression in reuse_info.spdx_expressions:
                ...
                if wrong_identifier:
                    _logger.warning(
                        ...
                        ).format(path=path, identifier=wrong_identifier)
                    )
    ...
    def bill_of_materials(
        self, out=sys.stdout, ignore_debian: bool = False
    ) -> None:
    ...
    def _file_information(
        self, path: PathLike, out=sys.stdout, ignore_debian: bool = False
    ) -> None:
        ...
        out.write("FileName: ./{}\n".format(relative))
        out.write(
            "SPDXID: SPDXRef-{}\n".format(hashlib.sha1(encoded).hexdigest())
        )

        with path.open("rb") as fp:
            out.write(
                "FileChecksum: SHA1: {}\n".format(_checksum(fp, hashlib.sha1))
            )
